...
import { ZoomLabel as PrismaZoomLabel } from "@/prisma/generated_schema/Example";
...
const ZoomLabelForm = (props: ZoomLabelFormProps) => {
    ...
    useEffect(() => {
        fetch('/api/examplePage/ZoomLabel', {
            method: 'GET',
            headers: {
    ...
    const fetchData = async () => {
        setIsLoading(true);
        try {
            await fetch('/api/examplePage/ZoomLabel')
            .then((response) => {
    ...
    const deleteLabel = async (label: PrismaZoomLabel) => {
        try {
            await fetch('/api/examplePage/ZoomLabel/' + label.id, {
                method: 'DELETE',
                headers: {
        ...
        onSubmit: async (values) => {
            if(values.title?.length > 0) {
                ...
                if(selectedLabel == null) {
                    ...
                    try {
                        await fetch('/api/examplePage/ZoomLabel/' + selectedLabel.id, {
                            method: 'PUT',
                            headers: {
                                'authorization': props.authToken,
                                'Content-Type': 'application/json',
